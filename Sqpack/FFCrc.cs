...
namespace Sqpack {
    public static class FFCrc {
        public const uint CrcInitialSeed = 0xFFFFFFFF;
        ...
        private static uint Compute(uint seed, byte[] buffer, int start, int size) {
            if(buffer == null)
                throw new ArgumentNullException(nameof(buffer));
        ...
        public static string ComputeToHex(string value) {
            return BitConverter.GetBytes(Compute(value)).Reverse().ToArray().ToHex();
        }

        private static readonly Dictionary<string, byte> RootToSqMap = new Dictionary<string, byte> {
            {
                "common", 0x00
            }, {
                "bgcommon", 0x01
        ...
        private static readonly Dictionary<byte, string> SqToRootMap = RootToSqMap.ToDictionary(_ => _.Value, _ => _.Key);
